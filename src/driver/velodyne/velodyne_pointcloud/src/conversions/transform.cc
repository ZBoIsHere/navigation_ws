...
void Transform::processScan(
  ...
  velodyne_rawdata::VPointCloud::Ptr outMsg(
      new velodyne_rawdata::VPointCloud());
  ...
  for (size_t next = 0; next < scanMsg->packets.size(); ++next) {
    ...
    try {
      ROS_DEBUG_STREAM("correcting distortion relative to "
                       << config_.frame_id);
      // stamp in the header is microseconds unix time.
      ros::Time stamp(outMsg->header.stamp / 1000000,
                      (outMsg->header.stamp % 1000000) * 1000);
      pcl_ros::transformPointCloud(inPc_.pc->header.frame_id, stamp,
                                   *(inPc_.pc), config_.frame_id, tfPc_,
                                   listener_);
