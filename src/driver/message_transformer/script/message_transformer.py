...
class Topic2UDPConverter:
    def __init__(
        self, config, local_port, server_ip="192.168.1.120", server_port=43893
    ):
...
class UDP2TopicConverter:
    ...
    def add_topic(self, config):
        if self.code_mapping.has_key(config["command_code"]):
            self.code_mapping[config["command_code"]][
                config["topic_name"]
            ] = copy.deepcopy(config)
        else:
            self.code_mapping[config["command_code"]] = {
                config["topic_name"]: copy.deepcopy(config)
            }
        print (self.code_mapping)
        exec (
            "self."
            + self.get_topic_clear_name(config["topic_name"])
    ...
    def publish_msg(self, command_code, data):
        ...
        if not self.code_mapping.has_key(command_code):
            raise Exception(
                "Receive command_code %d from UDP which is undefined in config file!"
                % (command_code)
            )
        ...
        for topic_name in self.code_mapping[command_code]:
            ...
            print (
                "self."
                + self.get_topic_clear_name(topic_config["topic_name"])
                + "_pub.publish(msg)"
            )
            exec (
                "self."
        ...
        while not rospy.is_shutdown():
            ...
            if is_ready[0]:
                ...
                if not data:
                    ...
                    if (
                        msg_type != 1
                        or msg_size + 12 != bytes(data).__len__()
                        or msg_size == 40
                    ):
                        continue
...
if __name__ == "__main__":
    ...
    for config in config_list:
        ...
        if config["convert"]["src"] == "topic" and config["convert"]["des"] == "udp":
            topic_coverter_list.append(
                Topic2UDPConverter(config, local_port=local_send_port)
            )
            local_send_port += 1
