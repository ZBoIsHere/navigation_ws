...
void update_piece_map(const std::string file_name,
                      pcl::PointCloud<Point> current_pointcloud) {
  // 若file已存在,更新; 否则,新建.
  std::mutex mu;
  std::unique_lock<std::mutex> q(mu);
  // current_pointcloud.points.resize(current_pointcloud.width *
  // current_pointcloud.height);
  std::cout << "height = " << current_pointcloud.height << std::endl;
  std::cout << "width  = " << current_pointcloud.width << std::endl;
  std::cout << "height*width = "
            << current_pointcloud.height * current_pointcloud.width
            << std::endl;
...
void LidarMapping::param_initial(ros::NodeHandle &nh,
  ...
  privateHandle.param<float>("passthrough_ground_height",
                             passthrough_ground_height, -1.0);
  ...
  privateHandle.param<double>("voxel_leaf_size", voxel_leaf_size,
                              0.5);  //对输入的 一帧点云做第一次降采样
  privateHandle.param<double>("costmap_voxel_size", costmap_voxel_size,
                              0.1);  //输出用于costmap的点云地图的降采样
  privateHandle.param<double>("global_voxel_leafsize",
                              param_global_voxel_leafsize,
                              1.0);  //将一帧点云添加到 full_map之前降采样
  privateHandle.param<double>("min_scan_range", min_scan_range,
                              0.01);  ////提取一帧雷达中 自定义的区域
  ...
  privateHandle.param<double>("min_update_target_map",
                              param_min_update_target_map, 0.);
  ...
  std::cout << "min_update_target_map:" << param_min_update_target_map
            << std::endl;
  std::cout << "extract_length: " << param_extract_length << std::endl;
  std::cout << "extract_width: " << param_extract_width << std::endl;
  std::cout << std::endl;
  if (param_min_update_target_map == 0. || param_extract_length == 0. ||
      param_extract_width == 0.) {
  ...
  std::cout << "passthrough_ground_height: " << passthrough_ground_height
            << std::endl;
  ...
  privateHandle.param<std::string>("lidar_topic", _lidar_topic,
                                   "/velodyne_points");
  ...
  privateHandle.param<bool>("incremental_voxel_update",
                            _incremental_voxel_update, true);
  std::cout << "incremental_voxel_update: " << _incremental_voxel_update
            << std::endl;  // 是否在cpu_ndt里做update????

  // piece step params
  privateHandle.param<bool>("if_create_piece_map", param_if_create_piece_map,
                            false);
  privateHandle.param<double>("piece_step", param_piece_step, 10.0);
  privateHandle.param<std::string>("piece_map_folder", param_piece_map_folder,
                                   "undefined");
  ...
  try {
    ...
    tf_listener.waitForTransform(param_base_frame, param_laser_frame,
                                 ros::Time(0),
                                 ros::Duration(param_tf_timeout * 10),
                                 ros::Duration(param_tf_timeout / 3));
    tf_listener.lookupTransform(param_base_frame, param_laser_frame,
  ...
  tf_btol = (tl_btol * rot_z_btol * rot_y_btol * rot_x_btol)
                .matrix();      //雷达到机器人的静态变换矩阵
  ...
  privateHandle.param<bool>("is_publish_map_for_planning",
                            is_publish_map_for_planning, "false");
  privateHandle.param<std::string>("planning_map_folder", planning_map_folder,
                                   "None");
  if (is_publish_map_for_planning) {
    // TODO:: check 输出文件夹是否存在,若不存在则新建
    std::cout << "Will publish whole map (default filter with 0.1) to "
              << planning_map_folder << std::endl;
  }
  //********************  发布 用于生成costmap的点云地图 ***********************
  privateHandle.param<bool>("is_publish_map_for_costmap",
  ...
  if (is_publish_map_for_costmap) {
    // TODO:: check 输出文件夹是否存在,若不存在则新建
    std::cout << "Will publish map for costmap to " << cost_map_folder
              << std::endl;
  }
...
void LidarMapping::imu_odom_calc(ros::Time current_time) {
  ...
  double diff_time =
      (current_time - previous_time)
          .toSec();  // static声明的变量只会在第一次使用时被声明,因此不会被覆盖
  ...
  current_pose_imu_odom.roll +=
      diff_imu_roll;  // 更新current_pose_imu_odom相关,作为历史记录
  current_pose_imu_odom.pitch += diff_imu_pitch;
  current_pose_imu_odom.yaw += diff_imu_yaw;

  // odom信息处理,计算 --
  // xyz移动距离的计算,融合odom的速度(位移)信息和imu的转角信息
...
double LidarMapping::calcDiffForRadian(const double lhs_rad,
...
bool LidarMapping::pose2GeometryPose__(geometry_msgs::Pose &to,
                                       const pose &from) {
...
void LidarMapping::imu_calc(ros::Time current_time1) {
  static ros::Time previous_time1 =
      current_time1;  //局部变量的值在函数调用结束后不消失,而保留原值
  ...
  current_velocity_imu_x +=
      accX * diff_time;  // imu的速度值会通过slam进行修正,以避免累计误差 line806
  current_velocity_imu_y +=
      accY *
      diff_time;  // 或者说imu计算时所用的速度并不是用imu得到的,而是用slam得到的
  current_velocity_imu_z +=
      accZ *
      diff_time;  // imu所提供的参数,主要用来计算角度上的偏移,以及加速度导致的距离上的偏移!@
  ...
  guess_pose_imu.x =
      previous_pose.x +
      offset_imu_x;  // previous_pose只有在完成一次ndt定位之后在更新previous_pose
  guess_pose_imu.y =
      previous_pose.y +
      offset_imu_y;  //    大约更新40次guess_pose_imu 才执行一次previous_pose
...
void LidarMapping::points_callback(
  ...
  pcl::PointCloud<Point>::Ptr transformed_scan_ptr(
  ...
  pcl::PointCloud<pcl::PointXYZI>::Ptr in_cloud_xyzp(
  ...
  pcl::PointCloud<Point>::Ptr scan_ptr(
      new pcl::PointCloud<Point>(scan));  // scan保存到scan_ptr中
  ...
  if (initial_scan_loaded == 0)  //如果是第一帧数据
  {
    pcl::transformPointCloud(*scan_ptr, *transformed_scan_ptr,
                             tf_btol);  // tf_btol为初始变换矩阵
    map += *transformed_scan_ptr;       // map   --用于ndt匹配的地图
    global_map +=
        *transformed_scan_ptr;  //存储全局地图 也就是我们最终保存的地图
  ...
  pcl::PointCloud<Point>::Ptr map_ptr(
      new pcl::PointCloud<Point>(map));  // 重要作用,用以保存降采样后的全局地图
  ...
  // ??在这里计算有意义吗???
  if (_use_imu && _use_odom) imu_odom_calc(current_scan_time);
  ...
  if (!_use_imu && _use_odom) odom_calc(current_scan_time);
  ...
  // start3 以下:根据guess_pose_for_ndt 来计算初始变换矩阵guess_init --
  // 针对TargetSource
  Eigen::AngleAxisf init_rotation_x(
      static_cast<const float &>(guess_pose_for_ndt.roll),
      Eigen::Vector3f::UnitX());
  Eigen::AngleAxisf init_rotation_y(
      static_cast<const float &>(guess_pose_for_ndt.pitch),
      Eigen::Vector3f::UnitY());
  Eigen::AngleAxisf init_rotation_z(
      static_cast<const float &>(guess_pose_for_ndt.yaw),
      Eigen::Vector3f::UnitZ());

  Eigen::Translation3f init_translation(
      static_cast<const float &>(guess_pose_for_ndt.x),
  ...
  if (_method_type == MethodType::use_pcl) {
    pcl_ndt.align(*output_cloud,
                  init_guess);  // pcl::aligin 需传入转换后的点云(容器),估计变换
    fitness_score = pcl_ndt.getFitnessScore();
    t_localizer =
        pcl_ndt
            .getFinalTransformation();  // t_localizer为ndt变换得到的最终变换矩阵(即source和target之间的变换)
    ...
    cpu_ndt.align(
        init_guess);  // cpu::align 只需要传入估计变换
                      // --建图的时候传入估计变换,定位matching的时候传入空的单位Eigen
  ...
  // 以确定是否更新全局地图 added_pose将一直定位于localMap的原点
  // ###############################################################################
  ...
  pcl::PointCloud<pcl::PointXYZI>::Ptr transformed_incloud(
      new pcl::PointCloud<pcl::PointXYZI>());
  if (is_publish_map_for_planning) {
    // 输入点云部分截取
    pcl::PointCloud<pcl::PointXYZI>::Ptr in_cloud_raw(
        new pcl::PointCloud<pcl::PointXYZI>());
    ...
    for (size_t i = 0; i < in_cloud_raw->points.size(); i++) {
      ...
      if (dist > 1.0 && dist < 10.0 && z < -1.3) indices.indices.push_back(i);
    }
  ...
  if (is_publish_map_for_costmap) {
    ...
    pcl::PointCloud<Point>::Ptr to_globalmap_for_costmap(
        new pcl::PointCloud<Point>());
    filter.convert(scan_ptr,
                   local_no_ground);  // 调用去地面filter导致帧率很慢,TODO::排查
    ...
    for (size_t i = 0; i < local_no_ground->points.size(); i++) {
      ...
      if (dist > 0.5 && dist < 30.0)  //一帧点云里可以用的数据
    ...
    voxel1.setLeafSize(costmap_voxel_size, costmap_voxel_size,
                       costmap_voxel_size);
    ...
    // }
    sensor_msgs::PointCloud2::Ptr costmap_cloud_ptr(
  ...
  if (shift >=
      min_add_scan_shift)  //更新 map  和  map_full   将新的一帧数据加上去
  {
    ...
    pcl::PointCloud<Point>::Ptr to_globalmap(
        new pcl::PointCloud<Point>());  // piece map也将使用该值
    ...
    voxel_grid_filter_tomap.setLeafSize(voxel_leaf_size, voxel_leaf_size,
                                        voxel_leaf_size);
    ...
    if (is_publish_map_full) {
      ...
      pcl::VoxelGrid<Point>
          global_voxel_grid_filter_tomap;  // filter to global-map(for save)
      voxel_grid_filter_tomap.setLeafSize(param_global_voxel_leafsize,
                                          param_global_voxel_leafsize,
                                          param_global_voxel_leafsize);
      voxel_grid_filter_tomap.setInputCloud(transformed_scan_ptr);
      voxel_grid_filter_tomap.filter(*to_globalmap);
      global_map += *to_globalmap;
      sensor_msgs::PointCloud2::Ptr msg_globalmap_ptr(
          new sensor_msgs::PointCloud2);
  ...
  if (shift >= param_min_update_target_map)  //更新 ndt的 InputTarget
  {
    ...
    {
      double len = std::sqrt(std::pow(point.x - current_pose.x, 2) +
                             std::pow(point.y - current_pose.y, 2));
    ...
    // sensor_msgs::PointCloud2::Ptr msg_debug_map_ptr(new
    // sensor_msgs::PointCloud2); pcl::toROSMsg(*target_map,
    // *msg_debug_map_ptr); debug_map_pub.publish(*msg_debug_map_ptr);

    if (_method_type == MethodType::use_pcl)
      pcl_ndt.setInputTarget(
          map_ptr);  // 注意:此时加入的target:map_ptr并不包括刚加入点云的transformed_scan_ptr
    else if (_method_type ==
             MethodType::
                 use_cpu)  // 只是说map更新了,因此target也要更新,不要落后太多
    {
  ...
  // matching_map_pub.publish(*map_msg_ptr); // "map_for_localmatch"
  // 每一帧都发布map(全局map)
  ...
  if (param_visualize) {
    ...
    // std::cout << "1-2: " << test_time_2 - test_time_1 << "s" << "--downsample
    // inputCloud" << std::endl; std::cout << "2-3: " << test_time_3 -
    // test_time_2 << "s" << "--set NDT params and inputSource" << std::endl;
    // std::cout << "3-4: " << test_time_4 - test_time_3 << "s" << "--handle
    // imu/odom and cal ndt resule" << std::endl; std::cout << "4-5: " <<
    // test_time_5 - test_time_4 << "s" << "--get current pose" << std::endl;
    // std::cout << "5-6: " << test_time_6 - test_time_5 << "s" << "--publish
...
void LidarMapping::try_to_create_piece_map(
    const pose cur_pose,
    const pcl::PointCloud<Point>::ConstPtr &to_global_map) {
  ...
  piece_map_container.points.resize(piece_map_container.width *
                                    piece_map_container.height);
  if (current_x_index != before_x_index || current_y_index != before_y_index) {
    file_name << param_piece_map_folder << "x" << before_x_index << "y"
              << before_y_index << ".pcd";
...
void LidarMapping::run(ros::NodeHandle &nh, ros::NodeHandle &private_nh) {
  ...
  // Eigen::Translation3f tl_btol(_tf_x, _tf_y, _tf_z);                 // tl:
  // translation  // tl_btol是初始设定的 Eigen::AngleAxisf rot_x_btol(_tf_roll,
  // Eigen::Vector3f::UnitX());  // rot: rotation Eigen::AngleAxisf
  // rot_y_btol(_tf_pitch, Eigen::Vector3f::UnitY()); Eigen::AngleAxisf
  // rot_z_btol(_tf_yaw, Eigen::Vector3f::UnitZ());

  // // 初始化tf_btol == 根据初始化得到的tl_btol等初始化tf_btol
  // -----以获得初始变换矩阵
  // ---当车辆初始的起点不在预定义的globalMap原点时,就需要tf_btol了 tf_btol =
  // (tl_btol * rot_z_btol * rot_y_btol * rot_x_btol).matrix(); tf_ltob =
  // tf_btol.inverse();  // 将tf_btol取逆
  // --因为我们后面配准的时候都是相对于localMap的原点的,因此tf_ltob.inv将作为补偿的矩阵
  ...
  debug_map_pub =
      private_nh.advertise<sensor_msgs::PointCloud2>("/deug_map", 1000);
  matching_map_pub = private_nh.advertise<sensor_msgs::PointCloud2>(
      "/globalmap/map_for_localmatch", 1000);
  refiltered_map_pub = private_nh.advertise<sensor_msgs::PointCloud2>(
      "/globalmap/map_full", 1000);
  current_pose_pub =
      private_nh.advertise<geometry_msgs::PoseStamped>("/current_pose", 1000);
  costmap_cloud_pub = private_nh.advertise<sensor_msgs::PointCloud2>(
      "/global_map/costmap_cloud", 1000);
  pub_imupose =
      private_nh.advertise<geometry_msgs::PoseStamped>("/imu_current_pose", 10);
  pub_path = private_nh.advertise<nav_msgs::Path>("path", 10, true);

  //		ros::Subscriber param_sub = nh.subscribe("config/ndt_mapping",
  // 10, param_callback); 		ros::Subscriber output_sub =
  // nh.subscribe("config/ndt_mapping_output", 10, output_callback);
  points_sub = private_nh.subscribe(_lidar_topic, 10,
                                    &LidarMapping::points_callback, this);
  odom_sub = private_nh.subscribe(_odom_topic, 1000,
                                  &LidarMapping::odom_callback, this);
  imu_sub =
      private_nh.subscribe(_imu_topic, 1000, &LidarMapping::imu_callback, this);
}
}  // namespace FAST_NDT
