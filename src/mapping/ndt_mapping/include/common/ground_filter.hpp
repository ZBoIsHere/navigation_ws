...
class RayGroundFilter {
 ...
 private:
  ...
  double local_max_slope_;    // degree default: 8 //max slope of the ground
                              // between points, degree
  double general_max_slope_;  // degree  default: 5 //max slope of the ground in
                              // entire point cloud, degree
  ...
  struct PointXYZIRTColor {
    ...
    int radial_div;  // index of the radial divsion to which this point belongs
                     // to
    int concentric_div;  // index of the concentric division to which this
                         // points belongs to
 ...
 public:
  RayGroundFilter() {
    ...
    minY = -1;
    maxY = 1;
    pub_debug_pc = nh.advertise<sensor_msgs::PointCloud2>(
        "RayFilter/velodyne_points_filtered", 10);
  };
...
#pragma omp for  // #pragma omp
                 // for语法OpenMP的并行化语法，即希望通过OpenMP并行化执行这条语句后的for循环，从而起到加速的效果。
  ...
  void remove_close_pt(double min_distance,
                       const pcl::PointCloud<PointNoI>::Ptr in,
                       const pcl::PointCloud<PointNoI>::Ptr out,
...
#pragma omp for
    for (int i = 0; i < in->points.size(); i++) {
      ...
      // else if(z < default_cluster_height - SENSOR_HEIGHT){   //
      // 过滤小半径范围内的地面点：
      // 因为这些点在射线上的距离很近;且近距离的地面点可简单通过阈值过滤掉
   ...
   */
  void XYZI_to_RTZColor(
      const pcl::PointCloud<PointNoI>::Ptr in_cloud,
    ...
    for (int i = 0; i < in_cloud->points.size(); i++) {
      ...
      auto theta = (float)atan2(in_cloud->points[i].y, in_cloud->points[i].x) *
                   180 / M_PI;
      if (theta < 0) {
        theta += 360;
      }
...
#pragma omp for
    for (int i = 0; i < in_radial_ordered_clouds.size();
         i++)  // sweep through each radial division 遍历每一根射线
    {
      float prev_radius = 0.f;
      float prev_height = -SENSOR_HEIGHT;
      bool prev_ground = false;
      bool current_ground = false;
      for (int j = 0; j < in_radial_ordered_clouds[i].size();
           j++)  // loop through each point in the radial div
      {
        float points_distance =
            in_radial_ordered_clouds[i][j].radius - prev_radius;
        float height_threshold =
            tan(DEG2RAD(local_max_slope_)) * points_distance;
        ...
        // check current point height against the LOCAL threshold (previous
        // point)
        if (current_height <= (prev_height + height_threshold) &&
          ...
          // check if previous point is too far from previous one, if so
          // classify again
  ...
  void convert(const pcl::PointCloud<PointNoI>::Ptr& current_pc_ptr,
    ...
    pcl::PointCloud<PointNoI>::Ptr cliped_pc_ptr(
        new pcl::PointCloud<PointNoI>());
    ...
    extract_adove.setIndices(
        boost::make_shared<pcl::PointIndices>(*up_indices));
    extract_adove.setNegative(
        false);  // true removes the indices, false save the indices
    ...
    XYZI_to_RTZColor(remove_close, organized_points, radial_division_indices,
    ...
    extract_ground.setIndices(
        boost::make_shared<pcl::PointIndices>(ground_indices));
  ...
  void setIfClipHeight(bool yn) { is_clip_height = yn; }
  ...
  void setMinDistance(double min_distance) { MIN_DISTANCE = min_distance; }
};  // end
};  // namespace utils
