...
static ros::Time callback_start, callback_end, t1_start, t1_end, t2_start,
    t2_end, t3_start, t3_end, ndt_start, ndt_end, t5_start, t5_end;
...
class LidarMapping {
 private:
  ...
  pose previous_pose, guess_pose, guess_pose_imu, guess_pose_odom,
      guess_pose_imu_odom;
  pose current_pose, current_pose_imu, current_pose_odom, current_pose_imu_odom;
  pose ndt_pose, localizer_pose;
  pose added_pose;  // added_pose记录点云加入地图时候的位置  //
                    // 初始设为0即可,因为第一帧如论如何也要加入地图的
  ...
  double diff_x, diff_y, diff_z,
      diff_yaw;  // current_pose - previous_pose // 定义两帧点云差异值
                 // --以确定是否更新点云等
  double offset_imu_x, offset_imu_y, offset_imu_z, offset_imu_roll,
      offset_imu_pitch, offset_imu_yaw;
  double offset_odom_x, offset_odom_y, offset_odom_z, offset_odom_roll,
      offset_odom_pitch, offset_odom_yaw;
  double offset_imu_odom_x, offset_imu_odom_y, offset_imu_odom_z,
  ...
  cpu::NormalDistributionsTransform<Point, Point>
      cpu_ndt;  // cpu方式  --可以直接调用吗??可以
#ifdef CUDA_FOUND
  gpu::GNormalDistributionsTransform gpu_ndt;
  // TODO:此处增加共享内存方式的gpu_ndt_ptr
  // std::shared_ptr<gpu::GNormalDistributionsTransform> gpu_ndt =
  ...
  double min_add_scan_shift;  // 定义将点云添加到locaMap里的最小间隔值
                              // --应该是添加到localMap吧??
 ...
 public:
  LidarMapping() {
    ...
    diff_x = diff_y = diff_z =
        0.0;  // current_pose - previous_pose // 定义两帧点云差异值
              // --以确定是否更新点云等
    ...
    Eigen::Matrix4f gnss_transform =
        Eigen::Matrix4f::Identity();  // 保存GPS信号的变量

    _use_imu = false;
    _use_odom = false;
    _imu_upside_down =
        false;  // 用以解决坐标系方向(正负变换)问题 (比如x变更为-x等)
    ...
    initial_scan_loaded =
        0;  // 用以确定是否为第一帧点云(第一帧点云不做匹配,直接添加到地图中去)
  ...
  // piece map functions
  void try_to_create_piece_map(
      const pose cur_pose,
      const pcl::PointCloud<Point>::ConstPtr &current_pointcloud);

  // void update_piece_map(const std::string file_name, pcl::PointCloud<Point>
  // current_pointcloud);
