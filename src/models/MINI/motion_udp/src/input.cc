...
int InputSocket::getPacket(uint8_t *pkt, size_t packet_size)
{
    ...
    while (true)
    {
        ...
        ssize_t nbytes = recvfrom(sockfd_, pkt,
                packet_size,  0,
        ...
        ROS_INFO_STREAM("incomplete packet read: "
                         << nbytes << " bytes");
    }
...
/** @brief Send one velodyne packet. */
int InputSocket::sendPacket(uint8_t *pkt, size_t packet_size)
{
    ssize_t nbytes = sendto(sockfd_, pkt, packet_size, 0, (struct sockaddr*)&client_addr,sizeof(client_addr));
