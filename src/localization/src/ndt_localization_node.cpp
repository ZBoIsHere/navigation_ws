...
bool NDTLocalization::init() {
  ...
  pnh_.param<std::string>("odom_frame", param_odom_frame_,
                          std::string("/odom"));
  pnh_.param<std::string>("base_frame", param_base_frame_,
                          std::string("/base_link"));
  pnh_.param<std::string>("laser_frame", param_laser_frame_,
                          std::string("/velodyne"));

  pnh_.param<std::string>("map_topic", param_map_topic_, std::string("/map"));
  pnh_.param<std::string>("odom_topic", param_odom_topic_,
                          std::string("/odomImu/odom_imu"));
  pnh_.param<std::string>("lidar_topic", param_lidar_topic_,
  ...
  pnh_.param<bool>("if_init_pose_with_param", param_init_pose_with_param,
                   false);
  ...
  pnh_.param<std::string>("global_map_file", map_file,
                          "Confirm Location of Global Map.");
  ...
  try {
    ...
    tf_listener_.waitForTransform(param_base_frame_, param_laser_frame_,
                                  ...
                                  ros::Duration(param_tf_timeout_ / 3));
    tf_listener_.lookupTransform(param_base_frame_, param_laser_frame_,
  ...
  tf_btol_ =
      (tl_btol * rot_z_btol * rot_y_btol * rot_x_btol)
          .matrix();  // Eigen::Matrix4f tf_btol_;  base_link  to  velodyne
  ...
  if (param_init_pose_with_param)
    ...
    sub_initial_pose_ = nh_.subscribe<geometry_msgs::PoseWithCovarianceStamped>(
        "/initialpose", 1,
        boost::bind(&NDTLocalization::initialPoseCB, this, _1));
...
/**
 * @brief 1. caculate pdf(probability density function)(mean, covariance) for
 * each voxel grid in model
 *
...
void NDTLocalization::update_target_map() {
  ...
  for (auto point : model_pc_.points) {
    double dist = std::sqrt(std::pow(point.x - current_pose_.x, 2) +
                            std::pow(point.y - current_pose_.y, 2));
  ...
  // publish target_map
  sensor_msgs::PointCloud2::Ptr msg_target_map_ptr(
...
void NDTLocalization::pointCloud_process() {
  ...
  try {
    base_laser_tf_listener_.lookupTransform(
        param_base_frame_, param_laser_frame_, ros::Time(0), base_to_laser);
