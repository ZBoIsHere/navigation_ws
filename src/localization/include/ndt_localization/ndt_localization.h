...
class NDTLocalization {
 public:
  NDTLocalization(ros::NodeHandle nh, ros::NodeHandle pnh)
      : nh_(nh), pnh_(pnh), target_map_ptr(new PointCloudT()) {}
  ~NDTLocalization();
  /**
   * @brief Initialize.
   *
  ...
  Eigen::Matrix4f newScan_Pose = Eigen::Isometry3f::Identity().matrix();
  Eigen::Matrix4f init_Pose = Eigen::Isometry3f::Identity().matrix();
  ...
  /**
   * @brief Get measured data points, estimate current pose using
   * 3D-NDT-matching.
   *
   * @param msg
   */
  ...
  static bool pubMarkerText(const ros::Publisher pub,
                            const geometry_msgs::Pose pose,
                            const ros::Time stamp, const std::string frame,
                            const std::string text);

  static bool pubMarkerCylinder(const ros::Publisher pub,
                                const geometry_msgs::Pose pose,
                                const ros::Time stamp, const std::string frame,
                                const geometry_msgs::Vector3 scale);

  static bool pubMarkerCube(const ros::Publisher pub,
                            const geometry_msgs::Pose pose,
                            const ros::Time stamp, const std::string frame,
                            const geometry_msgs::Vector3 scale);
